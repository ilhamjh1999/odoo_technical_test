...
class Material(models.Model):
    ...
    supplier_id = fields.Many2one('res.partner', string='Related Supplier', domain="[('supplier','=',True)]")

    @api.constrains('material_buy_price')
    def _check_buy_price(self):
        for record in self:
            if record.material_buy_price < 100:
                raise models.ValidationError('Material Buy Price should not be less than 100.')
